...
function Pending(){
    ...
    pending.forEach(value => {
        ...
        container += `<li style="font-family: 'Gill Sans', 'Gill Sans MT', Calibri, 'Trebuchet MS', sans-serif;font-size:larger; ">` + value.title + `</li>`;
        container += `<li style="font-style: normal; font-size:medium; padding-bottom:10px; " >` + value.description + `</li>`;
        container += `</ul>`;
        container += `</div>`;
        container += `<div class="ohk">`;
        container +=`<i class="fa fa-trash" type="submit" id="a" onclick="xyz('${value.id}')"  id='${value.id}'></i>`;
        container += `</div>`;
        container += `</div>`;
    });
